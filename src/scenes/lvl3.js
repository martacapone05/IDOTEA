...
let btn_r_muro;
let btn_e_funivia;
let vignetta_pecora; // NUOVO OGGETTO
...
function create(s) {
    ...
    PP.layers.set_z_index(vignetta_pecora, 24); // Dietro la pecora (25)
...
function update(s) {
    ...
    if (vignetta_pecora) {
        let dist_vig = Math.abs(player.geometry.x - 12627);
        if (dist_vig <= 800 && dist_vig > 20) {
